...
contains
    ...
    subroutine overlap_cis()
        ...
        ! -- Sort MOs.
        call sort_mo(occ1(:, 1), act1(:, 1), 1, moa1, s_mo_a)
        call sort_mo(occ2(:, 1), act2(:, 1), 2, moa2, s_mo_a)
        call sort_mo(occ1(:, 2), act1(:, 2), 1, mob1, s_mo_b)
        call sort_mo(occ2(:, 2), act2(:, 2), 2, mob2, s_mo_b)
        deallocate(occ1)
        deallocate(occ2)
        deallocate(act1)
        deallocate(act2)
        ...
        call freeze_mo_by_norm(freeze_mo_norm_t, 1, 1, moa1, s_mo_a, cisa1, n_freeze_a, .false.)
        call freeze_mo_by_norm(freeze_mo_norm_t, 1, 2, moa2, s_mo_a, cisa2, n_freeze_a, .true.)
        call freeze_mo_by_norm(freeze_mo_norm_t, 2, 1, mob1, s_mo_b, cisb1, n_freeze_b, .false.)
        call freeze_mo_by_norm(freeze_mo_norm_t, 2, 2, mob2, s_mo_b, cisb2, n_freeze_b, .true.)
